...
Финальный вывод теперь человекочитаемый блок с разделами:
```
Итоги выполнения:
Перспективные бумаги:
  В списке сейчас: 32
  Покрытие до: 32 (отсутствовало: 0)
  Новых добавлено: 0
  Покрытие после: 33 (лишних: 1)
  Процент покрытия: 100%
  Осиротевших до очистки: 1 (удалено строк: 250)
...
Экспорт:
  Статус: OK (файл: potentials_export.xlsx)
  JSON: potentials_export.json

Сводка: успешных этапов 5 из 5
```
Если основной Excel файл заблокирован (открыт в другой программе), включается fallback: создаётся файл с суффиксом `_lock` и отметкой времени.
...
| `FULL_REFRESH_PRUNE_ORPHANS` | 1 = удалять «осиротевшие» SECID из истории, которых нет в текущем списке perspective_shares | 0 |
...
При ошибке `PermissionError` (файл Excel занят) создаётся резервный файл: `<имя>_YYYYMMDD_HHMMSS_lock.xlsx`, а в отчёте выводится пометка о fallback.
